...
app.controller('IdentityCtl', ['$scope', '$stateParams', 'IdentityService',
  'ngDialog', 'FlowServiceV3', 'DegradeServiceV3', 'AuthorityRuleService', 'ParamFlowService', 'MachineService',
  '$interval', '$location', '$timeout',
  function ($scope, $stateParams, IdentityService, ngDialog,
    FlowServiceV3, DegradeServiceV3, AuthorityRuleService, ParamFlowService, MachineService, $interval, $location, $timeout) {
    ...
    function saveFlowRule() {
      if (!FlowServiceV3.checkRuleValid(flowRuleDialogScope.currentRule)) {
        return;
      }
      FlowServiceV3.newRule(flowRuleDialogScope.currentRule).success(function (data) {
        if (data.code === 0) {
          flowRuleDialog.close();
          let url = '/dashboard/v3/flow/' + $scope.app;
          $location.path(url);
        } else {
          alert('失败：' + data.msg);
        }
    ...
    function saveDegradeRule() {
        if (!DegradeServiceV3.checkRuleValid(degradeRuleDialogScope.currentRule)) {
            return;
        }
      DegradeServiceV3.newRule(degradeRuleDialogScope.currentRule).success(function (data) {
        if (data.code === 0) {
          degradeRuleDialog.close();
          var url = '/dashboard/v3/degrade/' + $scope.app;
          $location.path(url);
        } else {
          alert('失败：' + data.msg);
        }
