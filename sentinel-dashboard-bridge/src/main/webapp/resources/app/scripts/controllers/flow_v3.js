...
app.controller('FlowControllerV3', ['$scope', '$stateParams', 'FlowServiceV3', 'ngDialog',
  'MachineService',
  function ($scope, $stateParams, FlowServiceV3, ngDialog,
    MachineService) {
    ...
    function getMachineRules() {
      ...
      FlowServiceV3.queryMachineRules($scope.app, mac[0], mac[1]).success(
        function (data) {
    ...
    $scope.saveRule = function () {
      if (!FlowServiceV3.checkRuleValid($scope.currentRule)) {
        return;
      }
    ...
    function deleteRule(rule) {
      FlowServiceV3.deleteRule(rule).success(function (data) {
        if (data.code == 0) {
          getMachineRules();
          confirmDialog.close();
        } else {
    ...
    function addNewRule(rule) {
      FlowServiceV3.newRule(rule).success(function (data) {
        if (data.code == 0) {
          getMachineRules();
    ...
    function saveRule(rule, edit) {
      FlowServiceV3.saveRule(rule).success(function (data) {
        if (data.code == 0) {
          getMachineRules();
          if (edit) {
            flowRuleDialog.close();
